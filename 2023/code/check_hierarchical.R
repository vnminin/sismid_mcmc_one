...
# posterior predictive distribution of frequencies (n1,n11) 
# for comparison with the actually observed value.
#
# The function output is the posterior expectations of the four
...
# Thinning by 10
al = al[seq(1,length(al),10)]
be = be[seq(1,length(be),10)]

# The number of retained samples
#mcmc.subsamplesize = mcmc.size-mcmc.burnin
mcmc.subsamplesize = length(al)
...
#mcmc.sample = chain_hierarchical(mcmc.size=100000)
check_hierarchical(mcmc.sample,mcmc.burnin=20000)
