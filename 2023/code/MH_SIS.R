...
round(mean(la.retain),2)                            # marginal posterior mean
round(quantile(la.retain,c(0.05,0.95)),2)           # 90% posterior interval'

# mu
mu.retain = par[[2]][(burn.in+1):mcmc.size]
mu.retain = sort(mu.retain)
round(mean(mu.retain),2)                            # marginal posterior mean
round(quantile(mu.retain,c(0.05,0.95)),2)           # 90% posterior interval'
...
round(quantile(initprob.retain,c(0.05,0.95)),2)           # 90% posterior interval'
