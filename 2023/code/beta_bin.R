...
propose_pospar = function(cur_pospar, prop_unif, tuning_const){
  return(cur_pospar*exp(tuning_const*(prop_unif-0.5)))
}
...
mcmc_sampler = function(my_data, init_alpha, init_beta, prior_rate_alpha, prior_rate_beta, tuning_alpha, tuning_beta, mcmc_size, mcmc_burnin, mcmc_subsample){
  ...
  for (i in 1:mcmc_size){
    ...
    log_acc_alpha = log_alpha_posterior(new_alpha, cur_beta, cur_theta, my_data, data_sample_size, prior_rate_alpha)-log_alpha_posterior(cur_alpha, cur_beta, cur_theta, my_data, data_sample_size, prior_rate_alpha)+
